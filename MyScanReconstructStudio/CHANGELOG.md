...
## 2026-07-23 - Scan/Process 采集上下文方案同步（未修改代码）

- 明确 DLL/EXE 两种形态都使用统一采集服务和设备会话规则，不直接实现 USB、设备命令、解密或自动曝光。
- 规定跨进程或嵌入上下文必须记录机型系列和 Profile，设备编号/型号有则记录，并保留生产模式、固件、采集模式和扫描头。
- 详细方案同步到 `F:\MeyerScan\Documents\设备相关\数据采集-原始图像预处理方案.md`。

## 2026-07-15

- 版本升级为 `v0.1.4`；动态装载 ScanWorkflowUI/DataProcessUI 后先校验 API 版本，再获取并调用 C++ 虚接口。
- 保持 DLL/EXE 双形态和绝对路径加载，ABI 不兼容时记录错误并拒绝创建对应重资源页面。
