...
- 历史骨架的默认清单已同步到当前 31 个声明模块，包含设备采集四 DLL、`MeyerScan_UIResources.dll`、`ScanReconstructStudio.exe` 及扫描/处理/发送模块；如果运行目录存在 `config/version_modules.json`，仍以该 manifest 为准。
