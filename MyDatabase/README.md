...
Database 是非界面基础设施模块，当前不依赖 Qt。CaseOrderService、RuntimeDataCenter 等需要数据库的 Qt 服务/数据模块不得直接包含 `Database.h` 做 Qt 类型转换，应通过 `MyDatabaseQtAdapter` 集中转换；CaseUI、SettingsUI 等业务 UI 只接收宿主快照，不应持有 Database 或 Adapter。
