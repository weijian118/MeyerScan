...
当前默认清单记录 25 个启动文件，包含 MainExe、外部登录 DLL、自研基础模块、设备传输模块、统一 UI 资源模块、界面模块、扫描重建 EXE/DLL 双形态和发送模块。
