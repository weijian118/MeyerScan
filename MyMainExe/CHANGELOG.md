...
- 记录采集上下文传递字段：机型系列、Profile、设备编号状态、设备编号（有则记录）、设备型号/型号代码（有则记录）、生产模式、固件、采集模式和扫描头。
- 明确 MainExe 只编排准入和生命周期，不解析 B 包、不解密图像、不计算自动曝光。
- 明确颜色校准不检查标定器；三维校准后续接入标定器检查且当前不以完成状态作为准入门禁；所有采集 Profile 暂用 `queueDepth=64`。
