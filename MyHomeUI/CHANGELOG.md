...
- 版本升级为 `v0.3.3`，公共接口版本升级为 2；`Init()` 参数改为应用目录和日志目录，删除数据库配置语义。
- Logger/UIComponents 统一使用应用目录下绝对 DLL 路径，并在获取 C++ 接口前校验 ABI 版本。

## 2026-07-12

- 版本升级为 `v0.3.2`；动态加载 UIComponents 后检查 `Init()` 返回值，失败时清空共享接口并使用 HomeUI 本地 Qt 控件/QSS 降级，避免调用半初始化工厂。
- `HomeUITest` 增加 Init/CreateWidget 失败保护和独立退出码；测试宿主不会再对空 QWidget 调用 `findChildren()`。
- 补充共享组件初始化、跨 DLL UTF-8 缓冲区和测试清理顺序的中文实现注释；模块边界仍只负责首页入口展示和动作上报。
