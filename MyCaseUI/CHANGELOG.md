...
- `CaseUITest --smoke` 按当前“患者表格 + 订单卡片流”结构检查两类运行时数据，修复仍要求两个 QTableWidget 导致的误报失败。
- 版本升级为 `v0.3.0`：顶部订单/患者入口改为页面级单选导航，内部 `QTabWidget` 隐藏默认页签，避免重复导航。
- 默认订单页从临时表格骨架改为响应式卡片流；1920 宽度显示四列，1366 宽度自动降为三列，双击卡片继续只上报 `CaseActionOpenOrder`。
- 增加患者/订单/时间/日期筛选工具条、状态可读映射和空状态；正式数据仍只读 RuntimeDataCenter，不在 UI 内拼 SQL。
- CaseUITest 补足七条订单演示数据，并增加可选截图尺寸，用于验证卡片换行、状态色和长文本。
- 图标/QSS 正式发布改走 `MeyerScan_UIResources.dll`，不再要求客户目录存在散落图片和 QSS。
...
- 最终截图复核覆盖 1920x1080 四列和 1366x768 三列卡片流；中英品牌、六个顶部动作和筛选工具栏无重叠。
