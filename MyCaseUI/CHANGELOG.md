...
- 修正 Logger 生命周期边界：smoke 宿主传入日志目录时，由 CaseUI 初始化并关闭本地 Logger 会话；使用 `QLibrary::PreventUnloadHint` 避免退出阶段 DLL 卸载顺序问题。
...
- 界面可见文本使用 `QApplication::translate("CaseUI", "...")` 包装，为后续独立 `.qm` 翻译文件预留基础。
