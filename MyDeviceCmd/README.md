...
当前模块代码版本为 `0.6.1`，公共语义 API 版本为 `2.1.0`，公共结构 schema 和整数 ABI 版本均为 `4`。

## 当前范围

- 已按 `美亚无线口内扫描仪通讯协议-20250808.pdf` 实现 A 类帧编码、解码、长度检查、16 位求和校验和 0~3 字节零尾检查。
- 已按协议覆盖 48 组 A 类命令：设备编号（API 历史名 MachineCode）、主板版本、电池、设备信息/期限、灯光、相机参数、颜色矩阵、开窗位置、温度、帧率、固件擦除/分包烧写、两路相机标定、颜色标定和曝光参数。
...
- 颜色校准入口使用 `MeyerDeviceCmd_PrepareColorCalibration`，按“打开 Cypress 设备 -> 检查 USB3 -> `0xD4/0xD9` 读取 13 位设备编号 -> 编号未写入时用 `0xC2/0xC7` 探测系列 -> `0xCD/0xCE` 读取型号代码 -> 综合识别”执行。API 中 `MachineCode` 是历史协议命名，产品含义统一为设备编号。
...
`--preflight-real` 只执行颜色校准入口所需的只读链路：枚举 Cypress 设备、判断 USB2/USB3、发送 D4/D9、生产模式下发送 C2/C7、发送 CD/CE 并输出 reported/effective 身份记录。它不会调用设备信息写入、参数固化或 Flash 命令；返回码 `0` 表示可以继续（可能带兼容推断），`3` 表示被未连接、USB2、D9 回包异常、编号非法、身份冲突或 Profile 未知等门禁拦截。
