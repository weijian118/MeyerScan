...
当前模块代码版本为 `1.0.0`，公共语义 API 版本为 `3.0.0`，公共结构 schema 为 `7`，整数 ABI 版本为 `8`。
...
- 负责把 Capture Profile 的 `queueDepth=64`、B 包大小和接收超时显式传给 DeviceTransport，并把传输层超时、部分包、断连和 I/O 诊断原样提供给 CaptureService。
