...
对 `mOS MyScan 5/5H/6` 的采集 Profile，`0x0A` 开始采集后，DeviceCmd 与采集服务按以下规则协作：

- USB B 包由 Transport 持续接收；当前所有适配机型暂使用 `queueDepth=64`，请求环可以跨越组六图边界，不能暂停所有 IN 接收请求。
- 单图完成后由上层立即解密；整组状态汇总后，只有整组开灯才调用自动曝光算法。关灯组仍正常进入慢速后处理并发布 UI/算法结果。
- 一组间隔内最多发送两条无回包命令；自动曝光被跳过时仍可发送合法灯光命令。
- 第一条命令以 USB OUT 传输完成为当前发送成功条件，第二条命令前至少等待 `5 ms`。
- DeviceCmd 只负责发送和结果记录，不把 USB OUT 成功描述成设备内部已经应用参数。

每次命令和采集诊断必须携带：`deviceSeries`（必须）、`deviceProfile`（必须）、
`deviceIdStatus`（必须）、设备编号（有则记录）、设备型号/型号代码（有则记录）、
生产模式、固件、采集模式和扫描头。详细设计见
`F:\MeyerScan\Documents\设备相关\数据采集-原始图像预处理方案.md`。
