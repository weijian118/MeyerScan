...
## 2026-07-23 - v0.6.0 内部职责拆分

- 将原 `OrderCreateUIImpl.cpp` 拆为生命周期、上下文、布局、治疗方案、公共组件/服务集成和扫描流程六个翻译单元。
- 拆分只调整模块内部文件归属，`IOrderCreateUI`、动作 ID、建单 JSON、治疗方案和扫描流程行为保持不变。
- CMake、VS2015 手写工程、代码版本和 Windows 文件版本同步升级为 `0.6.0`。

## 2026-07-20

- 版本升级为 `v0.5.5`；“清空全部牙位”改用 UIComponents 警告级双按钮公共弹窗。
- 建单模块继续负责是否清空、动作回调和日志；公共弹窗只返回确认/取消结果。旧 UIComponents 缺少新导出时自动降级为 `QMessageBox`。
