...
## 2026-07-23 - 数据处理上下文方案同步（未修改代码）

- 明确 Process UI 只消费异步后处理结果，不连接设备、不重新读取设备信息、不创建采集线程。
- 规定处理结果必须保留机型系列和 Profile，设备编号/设备型号有则记录，并保留生产模式、固件、采集模式和扫描头。
- 明确后处理队列满时丢弃新副本的诊断状态不得被 UI 静默隐藏。
- 详细方案同步到 `F:\MeyerScan\Documents\设备相关\数据采集-原始图像预处理方案.md`。

## 2026-07-15

- 版本升级为 `v0.2.4`；新增公共接口 ABI 版本导出；流程显示只消费稳定步骤 `code`，不依赖上游翻译文字。
