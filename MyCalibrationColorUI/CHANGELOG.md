...
## 2026-07-23 - 颜色采集链路方案同步（未修改代码）

- 明确颜色校准 UI 不创建 USB 接收线程、不加载 DeviceTransport、不解析原始 B 包；采集由 CaptureService 统一编排。
- 明确颜色校准上下文必须记录机型系列和 Profile，设备编号/设备型号有则记录，并保留生产模式、固件、扫描头和 reported/effective 来源。
- 明确整组关灯时不进入自动曝光，后处理结果由独立副本异步传递给 UI。
- 明确关灯组仍正常接收慢速后处理结果并发布给 UI；颜色校准不做标定器连接检查。
- 详细流程和设备上下文合同同步到 `F:\MeyerScan\Documents\设备相关\数据采集-原始图像预处理方案.md`。
