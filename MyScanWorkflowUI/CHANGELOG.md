...
## 2026-07-23 - 扫描采集上下文方案同步（未修改代码）

- 明确扫描 UI 只消费 CaptureService 发布的结果，不直接访问 DeviceTransport/DeviceCmd，不实现解密、自动曝光和原始图像预处理。
- 规定扫描会话必须记录机型系列和 Profile，设备编号/设备型号有则记录，并随结果保留生产模式、固件、采集模式和扫描头。
- 详细方案同步到 `F:\MeyerScan\Documents\设备相关\数据采集-原始图像预处理方案.md`。

## 2026-07-15

- 版本升级为 `v0.2.4`；新增公共接口 ABI 版本导出；流程显示只消费稳定步骤 `code`，显示文字由本模块 `tr()` 转换。
