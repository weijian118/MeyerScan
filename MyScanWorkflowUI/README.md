...
- `SetSessionContextJson()` 只在 JSON 完整校验成功后替换缓存；非法 JSON 返回 false 并保留上一份有效流程。全部步骤禁用时保持无活动步骤，不强制进入第一个禁用步骤。
- `CreateWidget()` 只创建页面；宿主完成 QWidget 挂载后再显式调用 `Activate()`，离开前调用 `DeactivateAndRelease()`，避免创建阶段重复激活重资源。
...
- 测试宿主会传入自定义 `scanProcess.steps`，验证非法 JSON 不覆盖有效上下文、显式 Activate、顶部按钮来源、tooltip、手型光标和点击回调。
...
扫描 UI 后续只消费 `MyCaptureService` 发布的采集结果，不直接访问 `MyDeviceTransport` 或 `MyDeviceCmd`。每个扫描会话必须保留：
...
productionMode
firmwareVersion
captureMode=PracticeScan 或 OrderScan
scanHeadType
```

UI 不解析单图数据头、不执行解密、自动曝光、排序、镜像或减黑图。Scan 页显示的是后处理线程发布的最终结果；关灯组也必须正常显示后处理结果。减黑图由上层处理模块使用 `clamp(int(white) - int(black), 0, 255)`，设备上下文随结果按值传递，不能只保留型号显示文本。详细链路见 `F:\MeyerScan\Documents\设备相关\数据采集-原始图像预处理方案.md`。
