...
- `MyDeviceCmd / MeyerScan_DeviceCmd.dll` 是纯 C++ 设备协议语义层，负责协议表 48 组 A 类命令的编码、响应查找、长度/命令码校验、回包解析和固定 POD 转换，覆盖设备编号、产品身份、基础状态、灯光、相机/曝光/帧率、温度、两路相机标定、颜色矩阵/颜色标定、设备授权信息、固件擦除/分包烧写以及开始/停止采集编排；它通过显式绝对路径动态加载 DeviceTransport，不链接其 import lib。
...
- 普通已知 Profile 打开仍可使用 `modelHint/HostHint`；自动探测使用 `Unknown` 最小能力配置，固定执行“D4/D9 -> D9 校验失败时 C2/C7 -> CD/CE”。D9 无回包/普通坏包是回包异常，合法帧中的非法编号是编号异常，只有 D9 求和校验失败表示设备编号未写入并进入生产模式。
...
- DeviceCmd 只负责检测和生成带来源的兼容身份，不读取“创建/练习/校准”等 UI 模式。DeviceSessionHost/工作流层负责准入：只有创建订单扫描流程必须取得真实 reported 设备编号，否则返回稳定状态 `ProductionDeviceNumberRequired=14` 并关闭会话；练习、颜色校准和后续三维校准可使用生产设备的 effective 默认编号和型号。
...
- 颜色校准入口固定执行“工作台门禁 -> Cypress 自动枚举 -> 连接 -> USB3 -> D4/D9 -> 必要时 C2/C7 -> CD/CE -> 产品身份 -> 完整快照注入 -> 一次设备信息提示 -> 创建弹窗”。颜色校准不要求正式设备编号，生产设备可使用 effective 兼容身份；提示同时展示 effective 编号、具体产品、effective 型号代码、生产模式和兼容来源，reported 值写入 POD/日志。连接/USB/型号失败或证据冲突不得创建/闪现弹窗，关闭动作必须关闭会话。三维校准后续采用相同身份准入规则。
