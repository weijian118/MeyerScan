...
- 公共头、可编译实现、测试结果和已确认产品/协议资料属于事实来源；历史方案和注释不能覆盖当前代码事实。
- 架构推断必须标记为“建议”或“规划”，不能直接变成已落地模块、正式默认值或兼容分支。
- 产品行为、协议字节含义、字段增删兼容、跨模块数据所有权和客户裁剪策略存在多种合理解释时，修改前必须询问需求方。
- 未确认问题登记到进度文档的“待确认项”；确认后再同步任务总览、架构合同、代码、测试和模块 CHANGELOG。

## 2. 分层架构

下图表示目标依赖方向，同时包含已落地模块和规划能力；是否已经实现以任务总览模块清单和进度文档为准。
...
- 语言只在进程启动阶段读取 `application.language` 并安装各模块 `QTranslator`；用户修改后必须重启软件，下一次启动才生效。当前明确不实现运行时 `LanguageChange` 广播或页面即时刷新，登录 SDK 使用同一启动配置映射语言索引。
...
- 产品支持范围与协议识别范围分开：重构软件只支持 `mOS MyScan 5` 和 `mOS MyScan 6`。`mOS MyScan` 仍可被 ProductCatalog 精确识别并保留 D9/C7/CE 证据，但识别后必须在 ApplyDetectedModel、固件命令、校准和采集前返回稳定不支持状态并关闭会话。
...
- 颜色校准入口固定执行“工作台门禁 -> Cypress 自动枚举 -> 连接 -> USB3 -> D4/D9 -> 必要时 C2/C7 -> CD/CE -> 产品身份 -> 产品系列支持门禁 -> 14/15 主控板版本 -> 完整快照注入 -> 一次设备信息提示 -> 创建弹窗”。颜色校准不要求正式设备编号，受支持系列的生产设备可使用 effective 兼容身份；提示同时展示 effective 编号、具体产品、effective 型号代码、下位机版本、生产模式和兼容来源，reported 值写入 POD/日志。连接/USB/型号/必需版本失败、证据冲突或旧系列不支持时不得创建/闪现弹窗，关闭动作必须关闭会话。三维校准后续采用相同身份准入规则。
- 在上述身份和版本步骤之后，`mOS MyScan 5/5H/6` 先执行主控板颜色校准版本门禁：`1.1.x`、`1.2.x` 或无法解析的版本禁止进入颜色校准；满足要求后按 A3/A4 读取大扫描头、按 B9/BA 读取小扫描头。期望响应码正确但求和失败表示对应扫描头未校准，可以继续进入并提示；超时、错误帧头、错误响应码、截断或 payload 长度异常属于读取失败，禁止进入。旧 `mOS MyScan` 的 `LargeOnlyShared` 解析代码只保留给协议维护和诊断测试，正式预检在到达该步骤前已经拦截。结果集中记录在 `MeyerDeviceScanHeadColorCalibrationSnapshot`，MainExe/SettingsUI/CalibrationColorUI 只逐字段复制。
...
- 外部登录 SDK 的头文件、导入库、DLL、qm、许可和直接运行依赖保存在 `External/MyLoginSDK`，由 `MeyerScanExternalSdk.props/.cmake` 统一解析；兼容替代包只能通过 `MEYER_LOGIN_ROOT`/`MEYER_LOGIN_SDK_ROOT` 覆盖。
...
- 过大的实现文件在原模块内按稳定职责拆分翻译单元；拆分不能改变公共 ABI，也不能为了行数制造无职责类、无边界 DLL 或万能 Helper。
...
## 16. 设备采集与原始图像处理分层（2026-07-23 已确认）

设备采集是颜色校准、三维校准、练习扫描和创建扫描共同使用的基础能力，不能由任何一个 UI 模块单独实现。目标依赖关系为：

```mermaid
flowchart TD
    UI["ColorCalibrationUI / Calibration3DUI / ScanWorkflowUI"] --> Service["CaptureService\n采集线程和命令窗口"]
    Service --> Cmd["DeviceCmd\n设备命令、设备会话、Profile"]
...
所有设备命令和数据处理模块必须记录 `deviceSeries`（必须）、`deviceProfile`（必须）、`deviceId`（有则记录）、`deviceModel/modelCode`（有则记录）、`productionMode`、`firmwareVersion`、`captureMode` 和 `scanHeadType`。设备真实回包字段与流程兼容使用的 `effective` 字段必须分开保存。
...
完整图、线程、队列和 Profile 合同以 `F:\MeyerScan\Documents\设备相关\数据采集-原始图像预处理方案.md` 为准；汇报时可使用同目录的 `数据采集-原始图像预处理方案.docx` 或 Word 兼容版 `.doc`，二者不得脱离 Markdown 单独修改技术结论。
