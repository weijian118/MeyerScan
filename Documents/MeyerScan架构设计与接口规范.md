...
| 建单 UI | `MyOrderCreateUI` | `MeyerScan_OrderCreateUI.dll`，测试宿主 `OrderCreateUITest.exe` | DLL | 当前 v0.5.3 提供三栏自适应建单字段、五种修复类型、治疗方案图片/mask 交互、相邻牙桥连接点、扫描流程输入和三档截图验收；前四种类型以 b 图主色合成局部圆底后叠加白色 h 图，种植体由 QSS 绘制整行状态；扫描流程预览固定高度，不改变牙弓几何。 | 只提供 Order 步骤内容，不保存数据库、不决定流程、不把 bridge 当修复类型、不绘制工作台步骤导航；后续保存走 CaseOrderService / ScanSchemaService / OrderWorkflowService。 |
| 建单扫描工作台壳 | `MyOrderScanWorkspaceShell` | `MeyerScan_OrderScanWorkspaceShell.dll` | DLL / UI 容器 | 当前 v0.1.3 提供品牌、返回、唯一的 Order/Scan/Process/Send 步骤导航、最小化/关闭入口和步骤页面容器；练习模式只显示 Scan/Process。 | 只做容器和导航；内容页不得复制工作台步骤条；窗口动作上报 MainExe 执行。 |
...
### 14.10 自动测试、成熟度和可读性合同

1. 根 CMake 使用 CTest 登记各模块已有测试程序，不另写第二套测试逻辑。统一清单固定覆盖基础设施、服务、权限、资源、全部活跃 UI、ScanReconstructStudio，以及 MainExe 内部导航/第三方建单集成链路。
2. CMake 构建时重建 `build/ctest_runtime/<Config>`，先复制 MainExe 完整运行依赖，再覆盖同批次测试 EXE；CTest 不直接从可能残留旧兄弟 DLL 的模块 `bin/<Config>` 执行。测试默认自动结束，统一 `RUN_SERIAL` 并设置 120 秒 `TIMEOUT`；只有显式 `--show` 或截图模式可以进入人工事件循环。
3. 测试数据只写测试 EXE 所在目录下的隔离配置、数据库和日志；VersionManagerTest 继续使用 `test_runtime/VersionManagerTest`，不得覆盖 MainExe 正式 `version_modules.json`。
4. 测试层次必须区分：模块回归验证单个 DLL 合同；MainExe smoke 验证动态加载和页面编排；三档截图验证响应式 UI；真实数据库/设备/算法/安装包属于系统验收。低层测试通过不能替代高层验收。
5. 注释覆盖每个函数和关键代码段，并解释“使用了什么技术、为什么这样实现、对象/缓冲区由谁持有、失败后状态如何”。注释不得逐字复述赋值语句，也不得用历史 TODO 暗示 UI 可以直接读取 Database/ConfigCenter。
6. Qt 到 C ABI 的字符串转换固定为两种方式：有 QString 便捷重载时直接调用重载；否则先创建命名 `QByteArray`，在其作用域内传 `constData()`。被调用模块必须同步读取或立即复制，不能保存调用方指针。
7. 当前成熟度事实：首页/浏览/设置/建单/工作台/扫描/处理/发送页面及动态加载链路属于框架或最小实现；真实建单保存、配置持久化、设备/算法、校准计算、发送服务、IPC、更新和安装包仍未完成，后续不得根据页面存在推断业务闭环。

> **当前文档版本**：v1.33（2026-07-13，补齐统一测试、功能成熟度、分层验收和 UTF-8 缓冲区合同）
