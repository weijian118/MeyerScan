...
- `MyDeviceCmd / MeyerScan_DeviceCmd.dll` 是纯 C++ 设备协议语义层，负责协议表 48 组 A 类命令的编码、响应校验和固定 POD 转换，覆盖基础状态、灯光、相机/曝光/帧率、温度、两路相机标定、颜色矩阵/颜色标定、设备授权信息、固件擦除/分包烧写以及开始/停止采集编排；它通过显式绝对路径动态加载 DeviceTransport，不链接其 import lib。
- 公共结构必须包含 `structSize + schemaVersion + reserved`，只传固定宽度整数、UTF-8 和调用方缓冲区；C++ 异常、STL、CyAPI 类型不得越过 DLL 边界。
- 产品型号不能复用底层图像解码枚举。DeviceCmd 单独维护 `MyScan3/5/5H/6/6Wireless` 型号目录；5H 可复用 5 的协议配置但保留独立产品型号。只有 MyScan 6 Wireless 当前标记协议已核对，其余型号必须保持 `protocolVerified=0` 直到协议和实机验证完成。
- 普通已知机型打开仍可使用 `modelHint/HostHint`；颜色校准探测使用 `Unknown` 最小能力配置，读取 `0xCD/0xCE` 后只有在 337 字节预留区发现明确型号标记才切换为 `DeviceReported`。协议没有独立机型字段，禁止按设备编号前缀猜测或把 HostHint 伪装成设备自报型号。
...
- DeviceSessionHost 使用专用工作线程和串行命令队列；设置页、入口检查和校准只提交动作请求并读取不可变状态快照。状态快照必须带 `validFields`，未知/失败不能伪装成数值 0。
...
- 颜色校准入口固定执行“工作台门禁 -> Cypress 自动枚举 -> 连接 -> USB3 -> `0xCD/0xCE` -> 型号 -> 快照注入 -> 创建弹窗”；失败状态分别提示并立即返回，颜色校准关闭动作必须关闭会话。
...
- 设备编号可从 13 字节机器码/设备信息解析；期限码暂以原始十六进制返回，由后续授权/加解密层解释，DeviceCmd 不自行猜测期限业务含义。
