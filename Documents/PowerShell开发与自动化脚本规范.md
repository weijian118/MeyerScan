...
10. 当前安装的 VS2015 v140 编译器不支持 `/utf-8`。含中文注释的 `.h/.cpp` 必须保存为带 BOM 的 UTF-8；无 BOM UTF-8 会按代码页 936 解析，可能把多字节序列和换行错误组合，导致下一行代码被注释吞掉。
...
8. `catch {}` 中的 `$_` 是异常记录，不再是外层 `ForEach-Object` 的文件对象。进入 `try` 前先执行 `$path = $_.FullName`，`catch` 中只使用 `$path`，否则可能得到“Empty path name is not legal”。
...
3. 重构文档只维护仓库内 `Documents`，并由主 `/MIR` 与源码一起备份；禁止再引入外部文档源或 `_RefactorDocs` 重复快照，避免两份现行方案发生漂移。
4. 排除目录按深度从深到浅删除；参数使用字符串数组，保持 `/XD`、`/XF` 和值的顺序。
5. 资源生成、备份和清理脚本必须可重复执行。首次执行完成变更，第二次应无额外修改或提交。
...
> **文档版本**：v1.4（2026-07-14，补充 VS2015 中文源码编码和 catch 变量覆盖规则）
