...
| 设备命令 | `MyDeviceCmd` | `MeyerScan_DeviceCmd.dll` | 协议表 48 组 A 类命令编解码；提供颜色校准连接/USB3/`0xCD-0xCE`/型号预检、型号能力目录和设备状态快照 | 纯 C++；动态调用 DeviceTransport；固定 POD 跨 ABI；协议未定义独立机型字段，不按设备编号猜测型号；不创建 UI/算法/第二套连接 |
...
| 设置 | `MySettingsUI` | `MeyerScan_SettingsUI.dll` | General、Information、Calibration、Cloud、Scan、Process、About 页面；颜色校准入口先调用宿主预检，再承载半透明可拖动模态弹窗 | 不直连 Database/ConfigCenter/DeviceCmd；只传设备 POD 快照；保存和设备会话由宿主编排 |
...
| 颜色校准 | `MyCalibrationColorUI` | `MeyerScan_CalibrationColorUI.dll` | 接收已验证设备快照；独立无边框弹窗、可拖动标题栏、相机预览、校准/退出操作和计算接入点 | 不创建 DeviceCmd/Transport 句柄；无有效快照拒绝创建；设备取图与颜色计算尚未接入 |
...
- DeviceSessionHost 在专用工作线程串行执行查询和动作，缓存 `MeyerDeviceStateSnapshot`；UI 只读取带 `validFields` 的快照，不能在界面线程等待 USB。
- 颜色校准入口顺序固定为：工作台所有权门禁 -> 自动遍历 Cypress 设备 -> 未连接提示 -> USB2 提示 -> `0xCD` 请求/`0xCE` 响应 -> 解析明确机型标记 -> 注入快照 -> 创建校准弹窗。任何一步失败都不得创建/闪现校准弹窗。
- `0xCE` 的正式 382 字节布局只有加密、加密类型、13 字节设备编号、30 字节期限码和 337 字节预留区，没有独立机型字段。只允许从预留区明确标记或后续确认的集中规则识别型号，禁止按设备编号前缀猜测。
...
- MyScan 6 Wireless 协议已按 2025-08-08 文档核对；MyScan 3/5/5H/6 已进入型号目录但仍需各自协议和实机验证，不能仅因接口存在就标记完成。
