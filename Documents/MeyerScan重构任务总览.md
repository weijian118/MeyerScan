...
| 设备命令 | `MyDeviceCmd` | `MeyerScan_DeviceCmd.dll` | 协议表 48 组 A 类命令编解码和响应解析，覆盖基础状态、灯光、相机/曝光/帧率、温度、标定、设备授权信息、固件分包和采集启停；提供型号能力目录和状态快照 | 纯 C++；动态调用 DeviceTransport；固定 POD/调用方缓冲区跨 ABI；不创建 UI、不执行校准/重建算法、不允许各页面各开一套连接；固件完整升级策略由更新宿主负责 |
...
| 设置 | `MySettingsUI` | `MeyerScan_SettingsUI.dll` | General、Information、Calibration、Cloud、Scan、Process、About 页面；承载带半透明蒙层的可拖动颜色校准模态弹窗 | 不直连 Database/ConfigCenter；保存由宿主/服务编排；不实现校准业务 |
...
| 颜色校准 | `MyCalibrationColorUI` | `MeyerScan_CalibrationColorUI.dll` | 独立无边框弹窗、可拖动标题栏、相机预览、校准/退出操作、采集编排和计算接入点 | 不做三维校准和病例维护；设备取图与颜色计算尚未接入 |
