...
| 设备命令 | `MyDeviceCmd` | `MeyerScan_DeviceCmd.dll` | 50 个 A 类命令码编解码和集中回包解析；D4/D9 读设备编号、生产模式 C2/C7 探测系列、CD/CE 读型号代码、14/15 读主控板版本、mOS MyScan 用 12/13 读投图板版本；集中维护产品目录、reported/effective 检测记录和状态证据 | 纯 C++；动态调用 DeviceTransport；固定 POD 跨 ABI；兼容值不覆盖设备上报值；不创建 UI/算法/第二套连接 |
