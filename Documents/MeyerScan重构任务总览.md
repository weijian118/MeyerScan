...
| 设备命令 | `MyDeviceCmd` | `MeyerScan_DeviceCmd.dll` | 52 个 A 类命令码编解码和集中回包解析；D4/D9 读设备编号、生产模式 C2/C7 探测系列、CD/CE 读型号代码、14/15 读主控板版本、mOS MyScan 用 12/13 读投图板版本、MyScan 5/6 用 A3/A4 与 B9/BA 读取大小扫描头颜色校准状态；集中维护产品目录、reported/effective 检测记录和状态证据 | 纯 C++；动态调用 DeviceTransport；固定 POD 跨 ABI；兼容值不覆盖设备上报值；不创建 UI/算法/第二套连接 |
...
- 颜色校准入口顺序固定为：工作台所有权门禁 -> 自动遍历 Cypress 设备 -> 未连接提示 -> USB2 提示 -> D4/D9 -> 必要时 C2/C7 -> CD/CE -> 综合产品识别 -> 14/15 主控板版本 -> mOS MyScan 的 12/13 投图板版本 -> 注入完整快照 -> 一次设备信息提示 -> 创建校准弹窗。失败或冲突时不得创建/闪现校准弹窗。
- MyScan 5/6 在主控板版本读取成功后先执行 `1.3.x` 兼容门禁，再读取 A3/A4、B9/BA；A4/BA 期望回包校验失败是对应扫描头“未校准”，不是通信失败，大小扫描头状态随 POD 传递并按三种组合提示。MyScan 只使用大扫描头共享参数，不发送 B9/BA。
...
|---|---|---|---|
| mOS MyScan | SY-KS1000(P1) / P2 / P3 | 62000020 | 62000020 / 62010025 / 62010039 |
| mOS MyScan | mOS MyScan / mOS MyScan-P1 | 62000027 | 62000027 / 62010036 |
