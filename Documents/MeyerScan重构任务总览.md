...
>
> **设备文档入口**：机型使用指南、协议版本、适用机型、完整命令示例和设备调试技巧统一维护在 `F:\MeyerScan\Documents\设备相关`，不在本文件复制设备协议细节。
...
1. 先从权威文档、公共头、实现、测试、截图和已确认协议中建立事实清单。
2. 无法唯一确定产品行为、协议含义、数据归属、兼容范围或模块边界时，必须向需求方询问；不能用“通常如此”代替产品决定。
3. 需要临时假设才能继续验证时，假设必须显式标记、限制在测试代码或可替换配置中，并且不能进入正式默认行为。
4. 规划能力与已落地能力分开书写；模块存在、页面可显示或 smoke 通过，均不能推导出真实业务已完成。
...
|---|---|---|---|---|
| 主程序壳 | `MyMainExe` | `MeyerScan.exe` | 单实例、启动检查、登录、页面挂载、动态加载、导航、版本清单、唯一设备会话和工作流准入编排 | 不写业务 SQL、订单规则、设备协议或算法；设备检测仍委托 DeviceCmd |
...
| 设备传输 | `MyDeviceTransport` | `MeyerScan_DeviceTransport.dll` | 自动遍历 CyAPI USB 设备、严格识别 USB2/USB3、原始命令/流收发、64 深度异步 B 包请求环、传输诊断和旧完整帧兼容接口 | 纯 C++、x64；不解释设备命令或图像业务、不做 UI/重建算法；无线连接、串口/网口尚未实现 |
| 设备命令 | `MyDeviceCmd` | `MeyerScan_DeviceCmd.dll` | 52 个 A 类命令码编解码和集中回包解析；D4/D9 读设备编号、生产模式 C2/C7 探测系列、CD/CE 读型号代码、14/15 读主控板版本、MyScan 5/6 用 A3/A4 与 B9/BA 读取大小扫描头颜色校准状态；保留旧 MyScan 协议诊断和产品目录，但识别后统一返回不支持 | 纯 C++；动态调用 DeviceTransport；固定 POD 跨 ABI；只放行 mOS MyScan 5/6；兼容值不覆盖设备上报值；不创建 UI/算法/第二套连接 |
| 采集编排 | `MyCaptureService` | `MeyerScan_CaptureService.dll` | 快速采集线程、慢处理线程、准入、异常事件、命令窗口、后处理队列、算法选项快照和多路结果发布 | 纯 C++；动态加载四个下层 DLL；不实现图像算法；真实设备连续采集尚未联调 |
| 采集协议处理 | `MyCaptureProcessing` | `MeyerScan_CaptureProcessing.dll` | B 包/单图/组六图状态机、单图解密、状态汇总、排序、相机 1 镜像和饱和减黑图 | 纯 C++；只输出协议级标准化六图，不生成 RGB、不访问 USB |
| 采集图像流水线 | `MyCaptureImagePipeline` | `MeyerScan_CaptureImagePipeline.dll` | RGB888、标准化重建六图和后续显示/校准/AI/条纹多路输出 | 纯 C++；高级算法未接入时明确不可用，不伪造结果 |
| 自动曝光 | `MyAutoExposure` | `MeyerScan_AutoExposure.dll` | 会话级设备上下文、历史曝光状态和 16 字节曝光命令结果合同 | 纯 C++；当前算法未实现，返回 `NotImplemented`；不发送设备命令 |
...
| 共享 UI 组件 | `MyUIComponents` | `MeyerScan_UIComponents.dll` | 通用按钮、标签、输入、下拉、日期、表格、等待页，以及信息/成功/错误单按钮、警告/高危双按钮公共弹窗 | 不承载业务行为、权限和页面跳转；弹窗只返回选择结果；特殊控件留在业务模块 |
...
| 设置 | `MySettingsUI` | `MeyerScan_SettingsUI.dll` | General、Information、Calibration、Cloud、Scan、Process、About 页面；颜色校准入口调用宿主预检，一次展示设备/产品/兼容来源，再承载半透明可拖动模态弹窗 | 不直连 Database/ConfigCenter/DeviceCmd；只转发产品身份和完整检测 POD；保存和设备会话由宿主编排 |
...
| 颜色校准 | `MyCalibrationColorUI` | `MeyerScan_CalibrationColorUI.dll` | 接收含 reported/effective 编号/型号、D9/C7/CE 状态、主控板/投图板版本、生产/兼容来源、协议 Profile 和具体产品的已验证快照；提供无边框可拖动弹窗、预览和计算接入点 | 不解析原始回包或维护型号表，不创建 DeviceCmd/Transport 句柄；无有效快照拒绝创建；设备取图与颜色计算尚未接入 |
...
`HomeUI Create -> MainExe -> OrderScanWorkspaceShell -> OrderCreateUI -> ScanSchemaService -> MainExe -> CaseOrderService 最小保存 -> Scan/Process -> SendUI 动作上报`

目标链路（`OrderWorkflowService` 尚未开发）：

`OrderCreateUI -> OrderWorkflowService -> CaseOrderService + ScanSchemaService -> Scan/Process -> Send/DataExport`
...
当前已落地链路只覆盖 CaseUI 展示、动作上报和 MainExe 导航骨架，正式“打开/继续扫描”业务规则尚未闭环。

目标链路：`HomeUI Browse -> CaseUI -> CaseOrderService 查询 -> OrderWorkflowService 决策 -> MainExe 释放 CaseUI -> 扫描重建工作区`

进入扫描前必须销毁案例管理等非必要页面及缓存，给 VTK、OpenGL、算法和模型数据释放内存/显存空间。

### 4.5 设置

Home、Case、创建/练习的 Scan/Process 均可打开 SettingsUI。打开参数必须包含来源页；关闭后由 MainExe 根据来源刷新必要数据。创建/练习工作台占用设备时，点击校准必须提示退出扫描模块并返回；不能只靠隐藏/禁用按钮。设置持久化由 ConfigCenter/专用服务完成，SettingsUI 不直连配置文件、数据库或 DeviceCmd。
...
`Settings / Calibration / Scan 动作 -> MainExe 或独立扫描宿主的 DeviceSessionHost -> MyDeviceCmd -> MyDeviceTransport -> CyAPI USB`

- 本次重构软件的正式适配产品范围只有 `mOS MyScan 5` 和 `mOS MyScan 6`。旧 `mOS MyScan` 的协议、产品目录和诊断代码保留，但检测到该系列后统一提示“当前软件不支持该系列设备”，不得进入颜色/三维校准、创建扫描或练习扫描。
- 同一进程只允许设备会话宿主持有一个 `MeyerDeviceCmdHandle`；UI、校准和扫描页面不得各自创建 Transport/USB 连接。
- DeviceSessionHost 在专用工作线程串行执行查询和动作，缓存 `MeyerDeviceStateSnapshot` 和最近一次完整预检；UI 只读取 POD 副本，不能在界面线程等待 USB。
- 颜色校准入口顺序固定为：工作台所有权门禁 -> 自动遍历 Cypress 设备 -> 未连接提示 -> USB2 提示 -> D4/D9 -> 必要时 C2/C7 -> CD/CE -> 综合产品识别 -> 产品系列支持门禁 -> 14/15 主控板版本 -> 注入完整快照 -> 一次设备信息提示 -> 创建校准弹窗。失败、冲突或旧系列不支持时不得创建/闪现校准弹窗。
- MyScan 5/6 在主控板版本读取成功后先执行 `1.3.x` 兼容门禁，再读取 A3/A4、B9/BA；A4/BA 期望回包校验失败是对应扫描头“未校准”，不是通信失败，大小扫描头状态随 POD 传递并按三种组合提示。旧 MyScan 的大扫描头共享参数和投图板命令仅作为保留协议能力测试，不再进入重构软件业务流程。
- DeviceCmd API 中 `MachineCode` 是历史协议命名，业务含义统一为 13 位设备编号。编号前 8 位只能形成产品系列候选；完整 8 位型号代码才能确定具体产品。
- `0xCE` 至少存在旧有线“前 8 字节型号代码”和 MyScan 6 Wireless“授权信息”两种 382 字节布局，必须按协议 Profile 分支解析，禁止全局套用同一偏移或由 UI 自行解释。
- D9 命令码确认后，payload 长度 `0xFFFF` 与求和校验失败都视为生产阶段未写号，内部分别记录 `UninitializedLength` 和 `ChecksumIndicatesUnprogrammed`；无回包和普通坏包属于回包异常，合法帧中的空/全零/非 620000 前缀属于编号异常。生产模式继续用 C2/C7 探测系列，已知 C7 系列或编号前缀与 CE 型号代码不一致时返回 `Conflict`。
- 检测记录分别保存 reported/effective 编号和型号代码、值来源、生产模式、兼容标志及 D9/C7/CE 状态。CE 旧固件、坏包、校验/初始化/值异常可使用带来源的兼容值继续，但不得把兼容值写成设备真实上报。
- 创建/练习是否允许生产兼容身份分别由 `device.orderCreateAllowProductionMode`（默认 false）和 `device.practiceAllowProductionMode`（默认 true）控制；缺失配置时仍采用该安全默认。颜色校准和后续三维校准不受这两个工作台开关影响，可使用明确记录来源的 effective 默认身份。
- 工作台准入通过后由 MainExe 写入版本化 `deviceIdentity` JSON，必须同时保留 reported/effective、来源、生产/兼容标志、产品系列/型号/Profile 和检测状态；下游模块不得只复制 effective 值后丢弃来源。
- 已写合法编号但 CE 不可用时，兼容型号代码按已登记编号前缀选择同系列标准值 20/27/53/55，未知前缀才回退 20；禁止为了照搬单一默认值而跨系列套用校准 Profile。
- 设置页读取设备号、型号、版本、电池和期限原始信息；进入扫描前复用同一快照做连接/型号/期限检查；校准与扫描通过宿主申请互斥的设备工作模式。
- 采集中 A 类响应与 B 类图像可能共用 Bulk IN，禁止并发执行需要响应的状态查询；无响应的曝光等命令后续按具体机型协议逐项放开。
- MyScan 6 Wireless 协议已按 2025-08-08 文档核对；MyScan3/5/5H/6 是协议能力 Profile，不是完整销售产品名，其余 Profile 仍需各自协议和实机验证。

### 4.7.1 设备产品身份建模

识别结果必须同时保存 `protocolProfile`、`productFamily`、`productModel`、`identificationStatus` 和 `evidence`。状态至少包括 Unknown、SeriesOnly、ExactProduct、DeviceNumberUnprogrammed、Conflict；证据预留连接类型、编号前缀、型号代码、固件版本和命令能力。

| 产品系列 | 具体产品 | 设备编号前缀 | 型号代码 |
|---|---|---|---|
| mOS MyScan（仅识别诊断，不支持使用） | SY-KS1000(P1) / P2 / P3 | 62000020 | 62000020 / 62010025 / 62010039 |
| mOS MyScan（仅识别诊断，不支持使用） | mOS MyScan / mOS MyScan-P1 | 62000027 | 62000027 / 62010036 |
| mOS MyScan 5 | mOS MyScan 5 / mOS MyScan 5-P1 | 62000053 | 62000053 / 62010043 |
| mOS MyScan 5 | mOS MyScan 5H | 62000055 | 62000055 |
| mOS MyScan 5 | mOS MyScan 5-P2 龋齿检测版 | 62000053 | 待定 |
| mOS MyScan 6 | 国内有线版 / 国内无线版 | 待定 | 待定 |

识别顺序为连接类型候选 -> D4/D9 设备编号 -> 继续读取 CD/CE -> 按协议 Profile 选择回包布局 -> 完整型号代码精确映射 -> 校验证据冲突。仍无法精确识别时，后续再接固件版本或其它明确命令；在规则确认前保持 Unknown/SeriesOnly，禁止猜测。SeriesOnly 只能指导后续探测，不能进入依赖具体产品参数的颜色校准。
...
- 同进程、同 Qt/编译环境的模块内部可用 QString/QJson/std::string；公共 C ABI、跨进程和长期插件合同使用固定 UTF-8 字符数组、POD、调用方缓冲区或文件/订单 ID，不传 `std::string`、字符串数组、Qt/STL 容器所有权。
...
- 语言配置只在软件启动阶段读取并安装各模块 `QTranslator`；修改 `application.language` 后提示重启，下一次启动生效。禁止实现运行时 `LanguageChange` 广播、即时重建页面或让登录模块与主程序使用不同语言配置。
- 所有业务样式从 QSS 读取；业务源码禁止直接 `setStyleSheet()`，统一入口除外。
- 通用控件进入 UIComponents，单模块特殊控件留在自身模块；单按钮信息/成功/错误和双按钮警告/高危弹窗由 UIComponents 的独立 C ABI 提供，业务模块负责 `tr()`、日志和后续动作。只有形成独立复杂流程时再拆弹窗模块。
...
- 资源替换合同由 `Common/include/MeyerUiResourceContract.h` 唯一定义：资源 API `1`、RCDATA `101`、清单 schema `1`、qrc 前缀 `/MeyerScan/Modules`。Version.rc、C++ 加载器和生成脚本必须引用该合同，禁止重复硬编码。
- 客户资源补丁可以只覆盖 `MeyerScan_UIResources.dll`，但必须从该客户原交付 tag/worktree 构建、保留全部旧 alias、保持 Qt/MSVC/位数一致，并使用客户原 EXE 验证；新增资源不会自动被旧代码使用，替换既有图标必须保持原路径。
...
- Qt/VTK/OpenCV 和外部 SDK 路径集中在公共 props/cmake。登录 SDK 固定使用仓库 `External/MyLoginSDK` 或 `MEYER_LOGIN_ROOT` 显式覆盖，模块工程禁止保存 `D:\wj` 等个人绝对路径。
...
- 单个 `.cpp` 职责明显超过一个方向或接近千行时，优先在原模块内按生命周期、布局、数据、设备、协议等职责拆为小翻译单元；不为了缩短文件新增 DLL，也不把无关函数塞进 Common/Helper。
...
## 8. 设备采集和原始图像处理模块清单（2026-07-24 基础代码落地）

以下模块组成颜色校准、三维校准、练习扫描和创建扫描共用的采集链路。基础接口和模拟测试已落地；自动曝光、高级图像算法、真实设备连续采集和业务 UI 接入仍按各行状态单独验收。

| 模块中文名 | 项目名 | 规划产物 | 功能详情 | 设备信息要求 |
|---|---|---|---|---|
| 设备原始传输 | `MyDeviceTransport` | `MeyerScan_DeviceTransport.dll` | USB 连接、USB3 判断、Bulk IN/OUT、16384 字节 B 包持续接收、统一暂定 `queueDepth=64`、异步接收队列、传输错误和队列统计 | 记录机型系列和 Profile；设备编号/型号有则透传 |
| 设备命令服务 | `MyDeviceCmd` | `MeyerScan_DeviceCmd.dll` | 命令编码、响应解析、设备身份、固件、采集启停、曝光/灯光命令和采集命令串行调度 | 必须记录机型系列；设备编号、型号、型号代码有则记录；保留 reported/effective |
| 采集编排服务 | `MyCaptureService` | `MeyerScan_CaptureService.dll` | 采集线程、单图解密时序、六图组帧、整组状态、条件式自动曝光、两条命令窗口、后处理队列 | 每个采集会话必须带系列、Profile、编号状态、型号、模式和扫描头 |
| 采集数据处理 | `MyCaptureProcessing` | `MeyerScan_CaptureProcessing.dll` | 数据头解析、包序恢复、单图/整组处理、解密接口、图像排序、相机镜像、减黑图 | 每组输入输出必须带系列；编号/型号有则复制 |
| 采集图像流水线 | `MyCaptureImagePipeline` | `MeyerScan_CaptureImagePipeline.dll` | 标准化六图后的 RGB888 和显示/重建/校准等多路输出；高级算法缺失时显式不可用 | 每组输入输出必须带设备上下文、组序号和 options revision |
| 自动曝光算法 | `MyAutoExposure` | `MeyerScan_AutoExposure.dll` | 会话级上下文和输出合同已落地；后续按机型、帧率、扫描头和场景计算曝光参数 | 当前算法未实现；每次计算必须带系列、Profile、模式、扫描头 |
| 颜色校准界面 | `MyCalibrationColorUI` | `MeyerScan_CalibrationColorUI.dll` | 设备准入结果展示、采集控制和颜色校准交互 | 只消费宿主快照；必须显示/记录系列，编号/型号有则显示 |
| 三维校准界面 | `MyCalibration3DUI` | `MeyerScan_Calibration3DUI.dll` | 三维校准入口和结果展示 | 只消费宿主快照；必须记录系列，编号/型号有则记录 |
| 扫描界面 | `MyScanWorkflowUI` | `MeyerScan_ScanWorkflowUI.dll` | 创建/练习扫描部位显示、采集结果消费和扫描操作 | 每个会话消费不可变设备上下文，不重新探测设备 |
| 数据处理界面 | `MyDataProcessUI` | `MeyerScan_DataProcessUI.dll` | 处理采集结果、显示和后续工具入口 | 处理上下文必须携带系列、编号状态和型号 |
| 扫描工作台壳 | `MyOrderScanWorkspaceShell` | `MeyerScan_OrderScanWorkspaceShell.dll` | Order/Scan/Process/Send 统一承载和上下文转发 | 只转发设备上下文，不创建设备会话 |
...
### 8.2 当前明确的 Profile 规则

- MyScan 5/5H/6 使用独立 Profile。
- MyScan 6 当前初始值与 MyScan 5 25 帧模式相同，不共用变量。
- MyScan 5 支持 18/20/22/25 四种帧率 Profile；25 帧最短间隔约 25~30 ms，其他模式间隔更长。
- 每组六图间隔内最多两条无回包命令，命令之间至少 5 ms。
- 关灯组只跳过自动曝光，仍正常进入后处理并向 UI/算法发布结果。
- 减黑图使用 `clamp(int(white) - int(black), 0, 255)`，下限为 0、上限为 255。
- 后处理队列满时保留旧队列，丢弃新后处理副本；快速链路仍完成，开灯组自动曝光照常执行。
- 当前所有适配机型暂统一使用 `queueDepth=64`，必须由各自 Profile 显式提供。
- 颜色校准不检查标定器；标定器连接检查仅属于三维校准且后续接入。当前三维校准不以三维校准完成状态作为准入门禁。

详细架构图和验收条件见设备相关目录的 `数据采集-原始图像预处理方案.md`；面向评审和汇报的图文版为 `数据采集-原始图像预处理方案.docx`，并提供同内容的 `数据采集-原始图像预处理方案.doc` 兼容版本。
