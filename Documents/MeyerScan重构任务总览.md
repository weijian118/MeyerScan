...
| 设备命令 | `MyDeviceCmd` | `MeyerScan_DeviceCmd.dll` | 协议表 48 组 A 类命令编解码和集中回包解析；D4/D9 读设备编号、生产模式 C2/C7 探测系列、CD/CE 读型号代码；集中维护产品目录、reported/effective 检测记录和状态证据 | 纯 C++；动态调用 DeviceTransport；固定 POD 跨 ABI；兼容值不覆盖设备上报值；不创建 UI/算法/第二套连接 |
...
| 颜色校准 | `MyCalibrationColorUI` | `MeyerScan_CalibrationColorUI.dll` | 接收含 reported/effective 编号/型号、D9/C7/CE 状态、生产/兼容来源、协议 Profile 和具体产品的已验证快照；提供无边框可拖动弹窗、预览和计算接入点 | 不解析原始回包或维护型号表，不创建 DeviceCmd/Transport 句柄；无有效快照拒绝创建；设备取图与颜色计算尚未接入 |
...
- 颜色校准入口顺序固定为：工作台所有权门禁 -> 自动遍历 Cypress 设备 -> 未连接提示 -> USB2 提示 -> D4/D9 -> 必要时 C2/C7 -> CD/CE -> 综合产品识别 -> 注入完整快照 -> 一次设备信息提示 -> 创建校准弹窗。失败或冲突时不得创建/闪现校准弹窗。
...
- 只有创建订单扫描流程要求真实设备编号：创建模式进入 Scan/Process/Send 前必须有真实 reported 编号，未写号时返回状态 14、关闭会话并停止后续功能；练习、颜色校准和后续三维校准可使用系列探测后得到且明确记录来源的 effective 默认编号/型号。
