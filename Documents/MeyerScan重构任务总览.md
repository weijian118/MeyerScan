...
| 设备传输 | `MyDeviceTransport` | `MeyerScan_DeviceTransport.dll` | 自动遍历 CyAPI USB 设备、严格识别 USB2/USB3、原始命令/流收发、异步队列、图像包同步组帧和 IMU 解码 | 纯 C++、x64；不解释设备命令业务、不做 UI/重建算法；无线连接、串口/网口尚未实现 |
...
## 8. 设备采集和原始图像处理模块清单（2026-07-23 方案确认）

以下模块组成颜色校准、三维校准、练习扫描和创建扫描共用的采集链路。规划模块在代码实现前保持“规划中”状态，不得在进度中写成已完成。
...
| 自动曝光算法 | `MyAutoExposure` | `MeyerScan_AutoExposure.dll` | 按机型、帧率、扫描头和颜色/三维/扫描场景计算曝光参数，维护会话历史 | 每次计算必须带系列、Profile、模式、扫描头；编号/型号有则带入诊断 |
