...
不直接交给 UI。`MyDeviceTransport` 持续取走原始 B 包，`MyCaptureService` 负责快速链路、线程、队列和命令时序，`CaptureProcessing` 负责协议级标准化，`CaptureImagePipeline` 负责 RGB 和后续场景级多路处理，UI 只接收最终结果。这样颜色校准、三维校准、练习扫描和创建扫描共享同一套采集逻辑，UI 不重复创建线程和 USB 会话。
...
当前所有适配机型暂统一使用 `queueDepth=64`。它表示同时预提交的 USB IN 接收任务数量，每个任务对应一个 16384 字节 B 包，不是图像组数量，也不是后处理队列长度。请求环允许跨越组六图边界；每个任务完成后要及时重新提交，避免设备 USB 缓冲区被占满。当前没有第二个用户态原始包队列。后续如果某个机型需要不同值，必须只修改对应 Profile 并增加实机验证，不能依赖传输模块的隐式默认值。
