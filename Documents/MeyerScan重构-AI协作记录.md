...
### 2026-07-23：长期重构的事实、假设与确认门禁

- 问题：项目周期长、模块和协议持续扩展，仅凭局部代码或历史对话推断产品行为，容易把合理猜测写成正式方案并造成方向偏移。
- 结论：每轮修改先区分已确认事实、代码现状、合理推断和待确认项。产品行为、设备协议字段、数据所有权、模块边界、兼容策略和客户裁剪规则无法唯一确定时，必须先询问需求方。
- 边界：等待确认期间可以继续做不依赖该决定的静态检查、构建、测试和明确缺陷修复；不得新增正式默认值、协议分支或不可逆迁移。
- 影响：任务总览、架构规范、进度文档和后续 AI 协作均采用该门禁；规划能力与当前实现必须分开描述。

### 2026-07-23：设备适配范围收敛为 mOS MyScan 5/6

- 问题：重构软件预计 2027-10 投放时，旧 `mOS MyScan` 已停产约一年、用户少且可继续使用旧软件，为其继续承担重构适配和回归成本收益不足。
- 结论：正式产品只支持 `mOS MyScan 5` 和 `mOS MyScan 6`。旧系列协议、型号目录和检测证据保留，但产品识别完成后、ApplyDetectedModel/固件/校准/采集前返回稳定不支持状态并关闭会话。
- 影响：颜色校准、三维校准、创建和练习入口统一显示 `tr("Current software does not support this device series.")`；旧系列命令测试只证明协议代码可维护，不表示产品受支持。

### 2026-07-23：语言重启生效、登录 SDK 公共化和模块内拆分

- 语言：`application.language` 只在进程启动时读取，主程序各模块和登录 SDK 使用同一配置；修改后必须重启，不实现运行时 `LanguageChange` 广播或即时刷新。
- 外部 SDK：登录 SDK 交付物统一保存在 `External/MyLoginSDK`，CMake/VS2015 通过公共配置解析，禁止模块保存 `D:\wj` 等个人绝对路径。
- 可读性：大 `.cpp` 优先在原 DLL/EXE 内按职责拆翻译单元，保持公共 ABI；MainWindow、DeviceCommandService、OrderCreateUI 和 SettingsUI 已执行该规则，不新增无职责 DLL。
...
- 决定：D9 命令码确认后，`0xFFFF` 长度使用 `UninitializedLength`，求和校验失败使用 `ChecksumIndicatesUnprogrammed`；两者都设置生产模式并继续 C7/CE 和版本探测，但不填充 reported 设备编号。
- 验证：`DeviceCmdTest --smoke` 通过；实机只读预检得到 D9 `0xFFFF`、C7 MyScan 5/6 候选、CE 未初始化、effective 型号代码 `62000053`、主控板版本 `1.3.235`和 Ready，未进行写入操作。
- 影响：`MyDeviceCmd` 版本为 `0.7.1`；实机调试输出能区分“未初始化”与“校验失败”，创建流程仍可以根据正式设备编号门禁拒绝进入。

### 2026-07-22：命令兜底等待调整为 20 ms，并增加 MyScan 3 板间切换特例

- 问题：新 MyScan 3 在主控板 `0x15` 已成功返回后立即发送投图板 `0x12`，投图板回包连续缺失；单独发送 `0x12` 可以读到 `1.4.3992`，说明设备需要板间切换时间，而不是命令或投图板损坏。
- 决定：通用响应未完成兜底等待从 `50 ms` 调整为 `20 ms`；普通合法回包或业务可识别终态回包完成后仍不等待。MyScan 3 Profile 为 `0x14/0x15 -> 0x12/0x13` 增加独立 `20 ms` 切换等待，其它机型和其它命令不增加该等待。
- 验证：新设备 USB3、D9 返回正式编号 `6200002002566`、CE 未返回并使用兼容型号代码 `62000020`、产品型号识别为 `mOS MyScan/SY-KS1000(P1)`；主控板 `1.5.3993`、投图板 `1.4.3992` 均读取成功，颜色校准预检状态为 Ready。
- 影响：`MyDeviceCmd` 版本升级为 `0.7.7`，语义 API 升级为 `2.3.6`，schema 和整数 ABI 不变；增加 `profileSettleWaitUs` 诊断字段，不改变公共 ABI。

### 2026-07-21：命令间隔改为“未完成响应时的兜底窗口”

- 问题：此前把连续命令之间的 `50 ms` 误实现为每条后续命令发送前的固定等待，导致上一条命令已经收到并解析回包后仍被重复延迟。
- 决定：A 命令发送后，如果没有收到并解析出期望回包，B 命令发送前等待 `50 ms`，给 A 留出响应窗口；如果 A 已收到期望命令码，并解析为普通合法帧或业务可识别的终态回包，B 立即发送。D9/CE 的 `0xFFFF` 未初始化及已识别校验失败属于设备已经响应；无回包、接收超时、无法识别的坏帧和响应码不匹配仍保留等待标记。
- 边界：D4/D9、CD/CE 当前命令发送后等待 `50 ms` 再开始接收的规则继续保留，这是当前命令的接收时序，不是 A/B 之间的固定间隔；版本命令发送后直接阻塞接收。
- 影响：`MyDeviceCmd` 版本升级为 `0.7.5`，语义 API 升级为 `2.3.4`，schema 和整数 ABI 不变；时序诊断使用 `preSendWaitUs` 和 `postSendWaitUs` 区分两类等待。

### 2026-07-21：设备命令超时和命令间隔调整

- 问题：身份信息读取中 D4 回包前等待 100 ms、CE 回包前等待 200 ms，默认命令超时为 1500 ms，旧设备无回包时用户体感较慢。
- 决定：命令回包超时调为 500 ms；D4/D9、CD/CE 和版本请求发送后均等待 50 ms；真实 DeviceTransport 后端两条命令之间至少等待 50 ms，图像流超时仍独立保持 1500 ms。
- 影响：默认设备读取链路更快，模拟后端不人为延时；模块版本升级为 `0.7.2`，语义 API 为 `2.3.1`。
- 后续实机调试将低频命令超时进一步收紧为 `200 ms`，形成 `0.7.3/2.3.2`；上述旧版 50 ms 时序和 1500 ms 图像流超时不变。
- `0.7.4` 分阶段计时证明版本回包本身只需微秒至少量毫秒，耗时来自旧实现的“命令间隔 + 发送后等待”双重固定延时。因此版本命令曾暂时仅保留发送前的 50 ms 命令间隔，发送后立即阻塞接收；D4/CD 发送后的 50 ms 保持不变。该“固定命令间隔”表述已由 `0.7.5` 的响应窗口语义替代。
...
- 结论：固定检测顺序为连接/USB -> D4/D9 -> 生产未写号时 C2/C7 -> CD/CE -> 产品目录。D9 长度 `0xFFFF` 与求和校验失败都表示未写设备编号，但分别记录 `UninitializedLength` 和 `ChecksumIndicatesUnprogrammed`；无回包、普通坏包和合法帧中的非法编号分别返回明确失败状态。
...
- 结论：采集能力不放入颜色校准 UI；由独立采集编排服务统一管理线程、组帧、设备命令和快速链路。
- 结论：`MyDeviceTransport` 持续从 USB 端口取走 B 包，允许接收队列跨越组六图边界；不能为了发命令主动停止取包。
- 结论：单图完成后立即解密；六图完成后汇总状态，只有整组开灯才进入自动曝光。
- 结论：一组间隔内最多两条无回包命令，第一条 USB OUT 完成后等待 5 ms 再发第二条。
- 结论：排序、镜像、减黑图使用已解密数据副本异步处理；后处理队列满时丢弃新副本，不影响快速链路和自动曝光。
- 结论：整组关灯只跳过自动曝光，仍正常进入慢速后处理并向 UI/算法发布结果；减黑图使用 `clamp(int(white) - int(black), 0, 255)`。
- 结论：颜色校准不检查标定器；标定器检查仅属于三维校准且后续接入；当前三维校准不以完成状态作为准入门禁。
- 结论：所有当前适配机型暂使用 `queueDepth=64`，必须由各自 Profile 显式提供。
- 结论：MyScan 5/5H/6 使用独立 Profile；MyScan 6 暂用 MyScan 5 25 帧参数值，但不共用变量。
- 结论：设备命令和数据处理模块必须记录机型系列，设备编号/型号有则记录，并同时保留 reported/effective 来源。
- 详细图表和流程记录在 `设备相关/数据采集-原始图像预处理方案.md`；面向同事和领导汇报的图文版为同目录 `数据采集-原始图像预处理方案.docx`，`.doc` 为兼容分发版本。后续代码实现以 Markdown 基准为准。
...
> **文档版本**：v3.7（2026-07-24，同步采集四模块落地、场景级 Pipeline 和无用户态原始包队列）
