...
内部按生命周期/依赖、数据快照、页面布局和校准流程拆分翻译单元；公共 `ISettingsUI` 和校准 POD 合同保持不变。
...
- 检测到旧 mOS MyScan 系列时显示“当前软件不支持该系列设备”的翻译文本并返回，不创建颜色校准遮罩。
...
- **骨架期（当前 v0.9.0）**：路径使用 `QStandardPaths::DocumentsLocation` 派生安全默认值，云端地址保持空白提示；正式配置由 MainExe/设置服务读取 ConfigCenter 后通过版本化上下文注入，不显示开发机 `D:/` 路径，也不由 UI 直接读取配置文件；
