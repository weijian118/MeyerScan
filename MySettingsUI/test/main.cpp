...
// =============================================================================
// 文件说明:
//   SettingsUI 的界面冒烟测试宿主。
//
// 测试链路:
//   1. 在输出目录生成独立 SQLite 配置和测试数据库。
//   2. 写入医生、诊所、技工所等设置页需要展示的最小数据。
//   3. 初始化 SettingsUI，并验证 Information 页能从 RuntimeDataCenter 读出表格数据。
//
// 阅读重点:
//   - SettingsUI 不直接查询数据库，而是读取 RuntimeDataCenter 的只读快照。
//   - 测试库按 PID 命名，避免多个测试共用一个 SQLite 文件互相污染。
//   - 设置来源会影响校准入口，扫描重建来源应禁止校准。
// =============================================================================
